...
while splashScreenTimer < 100:
        ...
        startMessage = font_small.render("Jose Jardin", True, (171, 145, 123))
        screen.blit(startMessage, (screen.get_width()/2 - startMessage.get_width()/2, screen.get_height()/2 - startMessage.get_height()/2))
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == K_e:
                print(player.pos)
            if event.key == K_1:
                player_message = [180, 'Hi! its me again.', '']    
            if event.key == K_q:
                player_message = [180, 'You know who made me, right?', '']
            if event.key == K_w:
                player_message = [180, 'Yes, You are right, its Jose', '']
            if event.key == K_r:
                player_message = [180, 'Did you know that he is stupid at mathematics?', '']
            if event.key == K_t:
                player_message = [180, 'and also in english and science?', '']
            if event.key == K_y:
                player_message = [180, 'I dont know why he wants to take an engineering course.', '']
            if event.key == K_u:
                player_message = [180, 'Lol! HAHAHAH', '']                   
